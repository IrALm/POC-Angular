...
@Component({
  ...
  imports: [RouterLink],
  templateUrl: './proprietaire-dashboard.html',
})
export class ProprietaireDashboardComponent implements OnInit {
  ...
  readonly quittancesAlerte   = signal(0);
  ...
  ngOnInit() {
    forkJoin({
      ...
      next: ({ biens, contrats, quittances, edls }) => {
        this.biens.set(biens.slice(0, 3));
        ...
        this.quittancesAlerte.set(
          quittances.contenu.filter(q => q.statut === 'EN_RETARD' || q.statut === 'IMPAYEE').length
        );
  ...
  statutContratLabel(s: StatutContrat): string {
    const m: Record<StatutContrat, string> = {
      BROUILLON: 'Brouillon',
      EN_ATTENTE_SIGNATURE_PROPRIO: 'En attente (vous)',
